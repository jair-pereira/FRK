...
def run(problem, p, repetitions=2):
    print("Starting: ", problem.id, p)
    problem.observe_with(observer)
    for i in range(repetitions):
        print("Running: ", problem.id, i,"/",repetitions)
...
problem_param = zip(suite, params)
with Pool(processes=len(suite)) as pool:
    pool.map(run, problem_param, 1)
